<template>
  <div class="min-h-screen p-5">
    <header class="mb-6 flex items-center justify-between">
      <h1 class="text-3xl font-extrabold text-neon-amber">🍳 Cocina (KDS)</h1>
      <div class="flex items-center gap-3">
        <span class="badge bg-emerald-500/15 text-emerald-300">● en vivo</span>
        <span class="text-sm text-slate-400">{{ orders.kitchen.length }} comandas</span>
      </div>
    </header>

    <div
      v-if="orders.kitchen.length"
    ...
    <p v-else class="mt-20 text-center text-slate-500">Sin comandas de cocina pendientes 🎉</p>
  </div>
</template>
...
function urgencyClass(iso?: string) {
  ...
  if (min >= 15) return 'ring-2 ring-red-500/60';
  if (min >= 8) return 'ring-2 ring-amber-500/50';
  return '';
}
...
onMounted(async () => {
  if (auth.user?.businessId) orders.bindKitchen(auth.user.businessId);
  await orders.fetchKitchen().catch((e) => toast.error(apiErrorMessage(e)));
});
</script>
